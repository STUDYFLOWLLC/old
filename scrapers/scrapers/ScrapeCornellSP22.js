    ...
    await request(homeAddress, (error, response, html) => {
        if (!error && response.statusCode == 200) {
            ...
            for (let i=0; i<addresses.length; i++) {
                addressArray.push('https://classes.cornell.edu' + addresses[i].attribs.href)
            }
        }
    })
    return addressArray
    ...
    await request(address, (error, response, html) => {
        if (!error && response.statusCode == 200) {
            ...
            for (let i=0; i<codes.length; i++) {
                ...
                thisClass.instructor = instructors[i].children[instructors[i].children.length-1].prev.children[0].attribs ? instructors[i].children[instructors[i].children.length-1].prev.children[0].attribs['data-content'] : null 
...
const scrape = async (homeAddress, baseAddress) => {
    ...
    await Promise.resolve(
        getAddresses(homeAddress, baseAddress)
    )
    .then((value) => allAddresses = value)

    let allClasses = []
    for (let i=0; i<allAddresses.length; i++) {
        await 
            Promise.resolve(
                getDataFromAddress(allAddresses[i])
            )
            .then((value) => {
...
scrape('https://classes.cornell.edu/browse/roster/SP22', 'https://classes.cornell.edu')
